...
def get_minor_classes(major_class_idx):
    ...
    minor_class_indices = [Y_minor.columns.get_loc(col) for col in minor_classes if col in Y_minor.columns]  
    ...
    for class_idx in range(Y_major_pred.shape[1]):  
        if Y_major_pred[doc_idx, class_idx] == 0:
            ...
            for minor_class_idx in minor_class_indices:
                if minor_class_idx < len(Y_minor_pred_proba):
...
Y_minor_pred_full[:, [Y_minor.columns.get_loc(col) for col in Y_minor.columns]] = Y_minor_pred 
Y_proba_full = np.zeros((Y_minor_pred.shape[0], len(Y_minor.columns)))
Y_proba_full[:, [Y_minor.columns.get_loc(col) for col in Y_minor.columns]] = y_proba_matrix # 
